<template>
  <v-row justify="center">
    <v-col cols="2">
      <!-- 시도 셀렉트박스 -->
      <v-select label="시/도 선택"
                @change="_sidoChanged"
                v-model="region.sidoName"
                :items="getSidoListObserver"></v-select>
    </v-col>
    <v-col cols="2">
      <!-- 구군 셀렉트박스 -->
      <v-select label="구/군 선택"
                @change="_gugunChanged"
                v-model="region.gugunName"
                :items="getGugunListObserver"></v-select>
    </v-col>
    <!-- 동 셀렉트박스 -->
    <v-col cols="2">
      <v-select label="동 선택"
                ...
                :items="getDongListObserver"></v-select>
    </v-col>
