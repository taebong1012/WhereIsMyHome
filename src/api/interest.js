...
async function getInterestAptList(success, fail) {
  api_header.defaults.headers["access-token"] = sessionStorage.getItem("access-token");
  await api_header.get(`/interest`).then(success).catch(fail);
}

export { createInterest, deleteInterest, isExist, getInterestAptList };
